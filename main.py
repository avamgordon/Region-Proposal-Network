...
def resize_image_and_boxes(image_path, boxes):
    ...
    original_width, original_height = image.size # Storing the width and height of the input image to help scale the bounding boxes
    image = image.resize((TARGET_SIZE, TARGET_SIZE)) # Resizing the input image to be 200x200
    # Resize bounding boxes: locations should be divided by the original image sizes and then multiplied by the new sizes 
    for x, y, w, h in boxes:
        new_x = (x/original_width) * TARGET_SIZE # = (u_center/original_width) * new width
        new_y = (y / original_height) * TARGET_SIZE # = (v-center / original height) * new height
        new_w = (w / original_width) * TARGET_SIZE # = (width / original width) * new width
        new_h = (h / original_height) * TARGET_SIZE # = (height / original height) * new height
...
def compute_patch_grid():
    ...
    """
    grid = []
    w = PATCH_SIZE # Each patch has a width of 20 
    h = PATCH_SIZE # Each patch has a height of 20 
    y_center = PATCH_SIZE / 2 # y-coordinate center for (0,0) of grid
    x_center = PATCH_SIZE / 2  # x-coordinate center for (0,0) of grid 
    for i in range(GRID_NUMBER):        # i: column index (x-direction)
        for j in range(GRID_NUMBER):    # j: row index (y-direction, bottom-up)
            ...
            y_center += PATCH_SIZE # Increment y_center when moving to the next row 
        x_center += PATCH_SIZE # Increment x_center when moving to the next column 
        y_center = PATCH_SIZE / 2  # Reset y_center when moving to the next column
    return grid
...
def boxes_overlap(box1, box2):
    ...
    """
    result = True
    x_center_1, y_center_1, width_1, height_1 = box1 
    x_center_2, y_center_2, width_2, height_2 = box2 
    x_left_box1 = x_center_1 - (width_1 / 2)
    x_left_box2 = x_center_2 - (width_2 / 2)
    x_right_box1 = x_center_1 + (width_1 / 2)
    x_right_box2 = x_center_2 + (width_2 / 2)
    y_bottom_box1 = y_center_1 - (height_1 / 2)
    y_bottom_box2 = y_center_2 - (height_2 /2)
    y_top_box1 = y_center_1 + (height_1 / 2)
    y_top_box2 = y_center_2 + (height_2 / 2)
    if (x_right_box1 < x_left_box2 or x_left_box1 > x_right_box2 or y_top_box1 < y_bottom_box2 or y_bottom_box1 > y_top_box2):
        result = False
    return result
...
def generate_gt_tensors(boxes):
    ...
    for i, j, cx, cy, w, h in grid:
        x1 = cx - w / 2.0
        x2 = cx + w / 2.0
        y1 = cy - h / 2.0
        y2 = cy + h / 2.0

        # Determine if any GT box's center lies within these patch boundaries.
        overlap = False
        for box in boxes:
            x, y, w, h = box
    ...
    for j in range(GRID_NUMBER):
        for i in range(GRID_NUMBER):
            ...
            if existence[j, i, 0] > threshold:
                ...
                x_center, y_center, w, h = location[j, i]
                x_center_2 = x_center * (original_w/TARGET_SIZE)
                y_center_2 = y_center * (original_h/TARGET_SIZE)
                w2 = w * (original_w/TARGET_SIZE)
                h2 = h * (original_h/TARGET_SIZE)
                boxes.add((x_center_2, y_center_2, w2, h2, existence[j, i, 0]))
    return list(boxes)
...
def compute_iou(box1, box2):
    ...
    cx1, cy1, w1, h1 = box1
    cx2, cy2, w2, h2 = box2
    x_left_box1 = cx1 - (w1 / 2)
    x_left_box2 = cx2 - (w2 / 2)
    x_right_box1 = cx1 + (w1 / 2)
    x_right_box2 = cx2 + (w2 / 2)
    y_bottom_box1 = cy1 - (h1 / 2)
    y_bottom_box2 = cy2 - (h2 /2)
    y_top_box1 = cy1 + (h1 / 2)
    y_top_box2 = cy2 + (h2 / 2)
    x_left_intersect = max(x_left_box1, x_left_box2)
    x_right_intersect = min(x_right_box1, x_right_box2)
    y_bottom_intersect = max(y_bottom_box1, y_bottom_box2)
    y_top_intersect = min(y_top_box1, y_top_box2)
    area_of_overlap = (max(0, x_right_intersect - x_left_intersect)) * (max(0, y_top_intersect - y_bottom_intersect))
    box1_area = h1 * w1 
    box2_area = h2 * w2 
...
def match_anchors_to_ground_truth(resized_boxes, grid, anchor_shape):
    ...
    for gt in resized_boxes:
        for i, j, x_center, y_center, w, h in grid:
            anchor_height = hs * PATCH_SIZE
            anchor_width = ws * PATCH_SIZE
            anchor = [x_center, y_center, anchor_width, anchor_height] # Place an anchor box at the patch center
            IoUScore = compute_iou(anchor, gt) # Compute IoU between the anchor and all GT boxes 
            if IoUScore > best_iou_map[j, i]: # Track the best IoU for each location 
                best_iou_map[j, i] = IoUScore
                if IoUScore > 0: # if the current anchor has the best IoU and is greater than zero 
                    existence[j,i]=[1,0] # Set Existence[row, col] = [1,0]
                    # Encode the GT box offsets [dx, dy, dw, dh] from the anchor box:
                    dx = gt[0] - x_center
                    dy = gt[1] - y_center
                    dw = gt[2] - anchor_width
                    dh = gt[3] - anchor_height 
            else:
                existence[j, i] = [0,1]
    return existence, location
...
def decode_anchor_grid_predictions(existence, location, anchor_shape, original_size, threshold):
    ...
    for i, j, x_center, y_center, w, h in grid:
        c = existence[j, i, 0] 
        if c > threshold: # decode the box 
            anchor_width  = w * ws
            anchor_height = h * hs
            dx, dy, dw, dh = location[j, i]
            # Apply offsets to get [x_center, y_center, wicth, height]
            x_center = x_center + dx 
            y_center = y_center + dy 
            width = anchor_width  + dw 
            height = anchor_height + dh
            # Scale decoded box from resized_image to original image size 
            x_center_original = (x_center / TARGET_SIZE) * original_w
            y_center_original = (y_center / TARGET_SIZE) * original_h
            width_original = (width / TARGET_SIZE) * original_w
            height_original = (height/TARGET_SIZE) * original_h

            # Save the decoded prediction along with its confidence score.
            boxes.add((x_center_original, y_center_original, width_original, height_original, c))
    return boxes
...
def non_max_suppression(boxes, threshold=THRESHOLD):
    ...
    keep = [] # Empty set to record the subsampled boxes    
    # 1) Sort all the outputted boxes based on the "confidences" from high to low: 
    sorted_boxes = sorted(boxes, key=lambda box: box[4], reverse=True)
    for box in sorted_boxes: 
        if not any(compute_iou(box[:4], confident_box[:4]) >= threshold for confident_box in keep):
            keep.append(box)
    return keep
